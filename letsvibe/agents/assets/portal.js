...
(function () {
  ...
  function renderPlan(plan) {
    ...
    /* Highest-intent capture moment: right under the generated plan. */
    if (!(window.LVGrowth && LVGrowth.isSubscribed())) {
      html += '<div class="pcapture">' +
        "<h4>📬 Want plans like this in your inbox?</h4>" +
        "<p>Get the weekly LetsVibeAI briefing: new tutorials, tools, and curriculum updates matched to what you're learning.</p>" +
        '<div data-newsletter="curriculum-plan"></div>' +
      "</div>";
    }

    $("#curriculum-result").innerHTML = html;
    if (window.LVGrowth) LVGrowth.mountForms();
